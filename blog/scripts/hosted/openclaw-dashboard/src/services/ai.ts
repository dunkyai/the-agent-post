...
import {
  ...
  contactsSearch,
  docsCreate, docsRead, docsAppend, docsInsert, docsSuggestEdit, docsFormatText, docsReplaceText,
    ...
    switch (toolName) {
      ...
      case "docs_format_text":
        return await docsFormatText(input.document_id, input.text, {
          ...
          fontSize: input.font_size, fontFamily: input.font_family, foregroundColor: input.text_color,
        }, input.tab_id, acct);
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    if (data.stop_reason === "tool_use" && customToolUseBlocks.length > 0) {
      ...
      const googleToolNames = [
        ...
        "contacts_search",
        "docs_create", "docs_read", "docs_append", "docs_insert", "docs_suggest_edit", "docs_format_text", "docs_replace_text",
