...
// GET /oauth/buffer/callback — Buffer redirects here after user consent
router.get("/buffer/callback", async (req, res) => {
  ...
  if (error) {
    try {
      ...
      if (instance) {
        res.redirect(
          `https://${instance.subdomain}.agents.theagentpost.co/integrations?flash=Buffer+connection+cancelled`
        );
        return;
      }
    } catch {}
    res.status(400).send("Buffer connection was cancelled.");
    return;
  }
  ...
  try {
    ...
    if (!tokenRes.ok) {
      const body = await tokenRes.text();
      console.error("Buffer token exchange failed:", body);
      res.redirect(
        `https://${instance.subdomain}.agents.theagentpost.co/integrations?flash=Buffer+token+exchange+failed`
      );
      return;
    }
    ...
    // Deliver token to the instance
    const deliverRes = await fetch(
      `http://localhost:${instance.port}/webhook/buffer/tokens`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!deliverRes.ok) {
      console.error(
        "Buffer token delivery to instance failed:",
        await deliverRes.text()
      );
      res.redirect(
        `https://${instance.subdomain}.agents.theagentpost.co/integrations?flash=Failed+to+deliver+Buffer+tokens`
      );
      return;
    }

    console.log(`Buffer connected for instance ${instance.id}`);
    res.redirect(
      `https://${instance.subdomain}.agents.theagentpost.co/integrations?flash=Buffer+connected+successfully`
    );
  } catch (err) {
    console.error("Buffer OAuth callback error:", err);
    res.redirect(
      `https://${instance.subdomain}.agents.theagentpost.co/integrations?flash=Buffer+connection+failed`
    );
  }
});
